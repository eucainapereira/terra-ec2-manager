...
app.post('/api/instances/create', async (req, res) => {
  ...
  const { name, region, instanceType, ami, keyName, ports, userScript, adminPassword, generateKeyPair, isWindows, osType, tags } = req.body;
  ...
  const portList = Array.isArray(ports)
    ? ports.map(Number).filter(Boolean)
    : (typeof ports === 'string' ? ports.split(',').map(s => parseInt(s.trim())).filter(Boolean) : []);
  // Windows: auto-add RDP port 3389 if not present
  if (isWindows && !portList.includes(3389)) portList.push(3389);
...
app.get('/api/instances/:id/metrics', async (req, res) => {
  ...
  // InstanceID could be actual aws instanceId if it's external, or we extract from metadata if terraform
  ...
  try {
    ...
    const startTime = new Date(endTime.getTime() - 120 * 60 * 1000); // last 120 mins (2h)

    const getMetric = async (metricName) => {
      let stat = 'Average';
      if (metricName.includes('Network') || metricName.includes('Disk') || metricName.includes('EBS')) stat = 'Sum';
      if (metricName.includes('StatusCheckFailed')) stat = 'Maximum';
    ...
    const [cpu, netIn, netOut, diskRead, diskWrite, ebsRead, ebsWrite, health] = await Promise.all([
